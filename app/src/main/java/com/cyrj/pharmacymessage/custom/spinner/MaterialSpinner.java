...
package com.cyrj.pharmacymessage.custom.spinner;
...
import com.cyrj.pharmacymessage.utils.SprinnerUtils;
...
public class MaterialSpinner extends android.support.v7.widget.AppCompatTextView {
    ...
    private int ms_title_text_color; //标题字体颜色
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        boolean rtl = SprinnerUtils.isRtl(context);
        ...
        try {
            ...
            textColor = typedArray.getColor(R.styleable.MaterialSpinner_ms_text_color, Color.parseColor("#ffffff"));
            ms_title_text_color = typedArray.getColor(R.styleable.MaterialSpinner_ms_title_text_color,Color.parseColor("#666666"));
            ...
            arrowColorDisabled = SprinnerUtils.lighter(arrowColor, 0.8f);
        } finally {
            typedArray.recycle();
        }
    ...
    public void setBackgroundColor(int color) {
        ...
        if (background instanceof StateListDrawable) { // pre-L
            try {
                ...
                int[] colors = {SprinnerUtils.darker(color, 0.85f), color};
    ...
    public <T> void setItems(@NonNull List<T> items) {
        adapter = new MaterialSpinnerAdapter<>(getContext(), items).setBackgroundSelector(backgroundSelector).setTextColor(ms_title_text_color);
